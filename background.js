...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function startRecordingFlow() {
    ...
    try {
      ...
      // Choose optimal mimeType, prioritizing MP4 if supported
      let options = {};
      const mimeTypes = [
  ...
  function downloadVideo(video) {
    ...
    a.href = url;
    const extension = (video.blob && video.blob.type && video.blob.type.includes('webm')) ? 'webm' : 'mp4';
    a.download = `${video.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.${extension}`;
  ...
  function openPlayerModal(id) {
    getRecordingFromDB(id).then((video) => {
      ...
      const extension = (video.blob && video.blob.type && video.blob.type.includes('webm')) ? 'webm' : 'mp4';
      btnPlayerDownload.download = `${video.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.${extension}`;
